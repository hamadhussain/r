...
const Navbar = () => {
  ...
  const handleScroll = () => {
    if (window.scrollY > window.innerHeight && !isMobile) {
      setScrolled(true);  // Set scrolled state to true for larger screens
    } else {
      setScrolled(false);  // Reset the scrolled state
    }
  };
  ...
  return (
    <nav
      className={`bg-transparent bg-opacity-70 sm:fixed w-full bg-none px-2 sm:px-14 py-4 text--300 flex justify-center justify-between items-center ${
        (isMobile || isSmall) ? "bg-transparent" : scrolled ? "bg-transparent" : "bg-transparent"
      }`}
    >
